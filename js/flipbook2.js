...
function flipTo(next, forward) {
  ...
  requestAnimationFrame(function step(ts) {
    ...
    fl.style.boxShadow = forward
      ? `-${mid * 10}px 0 ${mid * 18}px rgba(0,0,0,${mid * 0.20})`
      : `${mid * 10}px 0 ${mid * 18}px rgba(0,0,0,${mid * 0.20})`;
...
function onMove(e) { if (!drag) return; const dx = (e.touches ? e.touches[0].clientX : e.clientX) - drag.x; if (Math.abs(dx) > 8) dragged = true; }
function onUp(e) {
  if (!drag) return;
  const dx = (e.changedTouches ? e.changedTouches[0].clientX : e.clientX) - drag.x;
  drag = null; if (!dragged) return;
  if (dx < -30) goNext(); else if (dx > 30) goPrev();
}
